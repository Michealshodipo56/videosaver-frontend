...
document.getElementById('mainDownloadBtn').onclick = () => {
    ...
    const downloadUrl = new URL(`${BACKEND_URL}/api/facebook/download`);
    downloadUrl.searchParams.set('url', urlInput.value.trim());
    downloadUrl.searchParams.set('kind', selectedOption.kind);
    downloadUrl.searchParams.set('format_id', selectedOption.format_id);
    downloadUrl.searchParams.set('title', title);
    downloadUrl.searchParams.set('label', label);
...
async function handleFetchMetadata() {
    ...
    if (!url) {
        showToast('Paste a Facebook video link first');
        return;
    }
    ...
    try {
        const response = await fetch(`${BACKEND_URL}/api/facebook/metadata?url=${encodeURIComponent(url)}`);
        ...
        if (!response.ok || data.error) {
            throw new Error(data.error || 'Failed to inspect Facebook video');
        }
...
function populateVideoInfo(data) {
    ...
    document.getElementById('videoTitle').textContent = data.title || 'Untitled Facebook Video';
    document.getElementById('platformBadge').textContent = data.platform || 'Facebook';
