...
document.getElementById('mainDownloadBtn').onclick = () => {
    ...
    const downloadUrl = `${BACKEND_URL}/api/x/download?` + new URLSearchParams(params).toString();
...
async function handleFetchMetadata() {
    ...
    if (!url) {
        showToast('Paste an X video link first');
        return;
    }
    ...
    try {
        const response = await fetch(`${BACKEND_URL}/api/x/metadata?url=${encodeURIComponent(url)}`);
        ...
        if (!response.ok || data.error) {
            throw new Error(data.error || 'Failed to inspect X video');
        }
...
function populateVideoInfo(data) {
    ...
    document.getElementById('videoTitle').textContent = data.title || 'Untitled X Video';
    document.getElementById('platformBadge').textContent = data.platform || 'X';
