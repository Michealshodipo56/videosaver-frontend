...
document.getElementById('mainDownloadBtn').onclick = () => {
    ...
    const downloadUrl = `${BACKEND_URL}/api/tiktok/download?` + new URLSearchParams(params).toString();
...
async function handleFetchMetadata() {
    ...
    if (!url) {
        showToast('Paste a TikTok video link first');
        return;
    }
    ...
    try {
        const response = await fetch(`${BACKEND_URL}/api/tiktok/metadata?url=${encodeURIComponent(url)}`);
        ...
        if (!response.ok || data.error) {
            throw new Error(data.error || 'Failed to inspect TikTok video');
        }
...
function populateVideoInfo(data) {
    ...
    document.getElementById('videoTitle').textContent = data.title || 'Untitled TikTok Video';
    document.getElementById('platformBadge').textContent = data.platform || 'TikTok';
