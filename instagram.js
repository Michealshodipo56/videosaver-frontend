...
document.getElementById('mainDownloadBtn').onclick = () => {
    ...
    const downloadUrl = `${BACKEND_URL}/api/instagram/download?` + new URLSearchParams(params).toString();
...
async function handleFetchMetadata() {
    ...
    if (!url) {
        showToast('Paste an Instagram video link first');
        return;
    }
    ...
    try {
        const response = await fetch(`${BACKEND_URL}/api/instagram/metadata?url=${encodeURIComponent(url)}`);
        ...
        if (!response.ok || data.error) {
            throw new Error(data.error || 'Failed to inspect Instagram video');
        }
...
function populateVideoInfo(data) {
    ...
    document.getElementById('videoTitle').textContent = data.title || 'Untitled Instagram Video';
    document.getElementById('platformBadge').textContent = data.platform || 'Instagram';
